#!/usr/bin/env bash
# Build an Vanilla WoW Launcher AppImage for Linux.
#
# Prerequisites:
...
#
# Produces: dist/VanillaWoWLauncher-x86_64.AppImage
set -euo pipefail
...
uv sync --dev
uv run pyinstaller --noconfirm --clean VanillaWoWLauncher-linux.spec
...
"$RENDER" 'VanillaWoWLauncher.ico[8]' -resize 256x256 \
  "$APPDIR/usr/share/icons/hicolor/256x256/apps/${APP_NAME}.png"

install -m 0755 packaging/linux/AppRun "$APPDIR/AppRun"
install -m 0644 packaging/linux/VanillaWoWLauncher.desktop "$APPDIR/VanillaWoWLauncher.desktop"
...
"$LINUXDEPLOY" --appdir "$APPDIR" \
  --desktop-file "$APPDIR/VanillaWoWLauncher.desktop" \
...
# to the canonical dist/VanillaWoWLauncher-<arch>.AppImage path.
