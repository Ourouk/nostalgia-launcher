#!/usr/bin/env bash
# Build an Nostalgia Launcher AppImage for Linux.
#
# Prerequisites:
...
#
# Produces: dist/NostalgiaLauncher-x86_64.AppImage
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
cd "$ROOT"

APP_NAME="NostalgiaLauncher"
ARCH="$(uname -m)"
...
uv sync --dev
uv run pyinstaller --noconfirm --clean NostalgiaLauncher-linux.spec
...
install -m 0644 packaging/linux/NostalgiaLauncher.desktop "$APPDIR/NostalgiaLauncher.desktop"
