...
def test_launch_settings_defaults(cfg, fakes):
    ...
    assert c.launch.umu_game_id == "umu-nostalgia-launcher"
...
def test_available_protons_includes_detected_builds(cfg, fakes, monkeypatch):
    monkeypatch.setattr(
        "nostalgia_launcher.services.umu.available_protons",
...
def test_set_umu_skip_builtin_dxvk_persists(cfg, fakes):
    c = SettingsController(
        fakes.dispatcher, fakes.updater, fakes.mods, fakes.addons, fakes.news
    )
    c.set_umu_skip_builtin_dxvk(True)
    assert cfg["launch"]["umu_skip_builtin_dxvk"] is True
    assert c.launch.umu_skip_builtin_dxvk is True
    c.set_umu_skip_builtin_dxvk(False)
    assert cfg["launch"]["umu_skip_builtin_dxvk"] is False
...
def test_linux_features_delegates_to_umu(cfg, fakes, monkeypatch):
    ...
    monkeypatch.setattr(
        "nostalgia_launcher.services.umu.scan_linux_features",
        lambda: features,
    )
    c = SettingsController(
        fakes.dispatcher, fakes.updater, fakes.mods, fakes.addons, fakes.news
...
def test_resolve_umu_binary_prefers_override(cfg, fakes, monkeypatch):
    ...
    c.set_umu_binary_path("")
    monkeypatch.setattr(
        "nostalgia_launcher.services.umu.find_umu",
