...
def test_discord_button_opens_configured_url(qapp, monkeypatch):
    ...
    try:
        opened = []
        monkeypatch.setattr(
            "nostalgia_launcher.ui.qt.main_window.webbrowser.open",
            opened.append,
        )
...
def test_terminate_readiness_shows_red_enabled_button(
    ...
    import nostalgia_launcher.controllers.update as update_controller
    from nostalgia_launcher.controllers.update import Readiness
...
def test_wordmark_shows_themed_logo_pixmap(qapp, tmp_path, monkeypatch):
    launcher.configure_from_dict(
        {
            ...
            "theme": {"logo": "https://server.example/logo.png"},
        }
    )
    ...
    monkeypatch.setattr(
        "nostalgia_launcher.services.logo.fetch_logo", lambda url: str(png)
    )
...
def test_wordmark_keeps_server_name_when_logo_fails(
    ...
    monkeypatch.setattr(
        "nostalgia_launcher.services.logo.fetch_logo", lambda url: None
    )
    ...
    try:
        ...
        assert win._wordmark.text() == "ExampleServer"
    finally:
        win.close()
