...
import nostalgia_launcher.services.umu as umu
...
def test_build_env_dxvk_d3d8_skip_builtin_swaps_to_async_flags(
    monkeypatch, tmp_path
):
    """Skipping the built-in DXVK omits PROTON_DXVK_D3D8 (so Proton never
    stages its stock DXVK over client-folder DLLs) and exports both async
    shader-compilation spellings for a dxvk-gplasync build."""
    env = umu.build_env(
        "UMU-Proton", "id", renderer="dxvk-d3d8", skip_builtin_dxvk=True
    )
    assert "PROTON_DXVK_D3D8" not in env
    assert env.get("DXVK_GPLASYNC") == "1"
    assert env.get("DXVK_ASYNC") == "1"
    assert "PROTON_USE_WINED3D" not in env


def test_build_env_skip_flag_noop_without_dxvk_renderer(monkeypatch, tmp_path):
    env = umu.build_env(
        "UMU-Proton", "id", renderer="auto", skip_builtin_dxvk=True
    )
    assert "PROTON_DXVK_D3D8" not in env
    assert "DXVK_GPLASYNC" not in env
...
def test_launch_captures_merged_child_output(monkeypatch, tmp_path):
    """umu/Wine output must reach the session log: the child is spawned with
    merged stdout+stderr pipes for the caller to drain."""
    game = tmp_path / "game"
    game.mkdir()
    exe = game / "WoW.exe"
    exe.write_text("")
...
def test_launch_forwards_wayland_env(monkeypatch, tmp_path):
    ...
    _, kwargs = popen.call_args
    assert kwargs["env"]["PROTON_ENABLE_WAYLAND"] == "1"


def test_launch_forwards_skip_builtin_dxvk_env(monkeypatch, tmp_path):
    game = tmp_path / "game"
    game.mkdir()
    exe = game / "WoW.exe"
    exe.write_text("")
    ...
    umu.launch(
        ...
        proton="UMU-Proton",
        renderer="dxvk-d3d8",
        skip_builtin_dxvk=True,
    )

    _, kwargs = popen.call_args
    assert "PROTON_DXVK_D3D8" not in kwargs["env"]
    assert kwargs["env"]["DXVK_GPLASYNC"] == "1"
...
def test_launch_spawns_umu_detached(monkeypatch, tmp_path):
    ...
    pid, pgid, proc = umu.launch(
        ...
        str(exe),
        proton="GE-Proton9-4",
        game_id="umu-nostalgia-launcher",
    )
    ...
    assert kwargs["env"]["GAMEID"] == "umu-nostalgia-launcher"
