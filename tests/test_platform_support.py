...
def test_config_dir_windows_uses_appdata(fake_platform, monkeypatch, tmp_path):
    ...
    assert config_dir() == str(tmp_path / "roaming" / "NostalgiaLauncher")
...
def test_cache_dir_windows_uses_localappdata(
    ...
    assert cache_dir() == str(tmp_path / "local" / "NostalgiaLauncher")
...
def test_cache_dir_windows_falls_back_to_appdata(
    ...
    monkeypatch.setenv("APPDATA", str(tmp_path / "roaming"))
    assert cache_dir() == str(tmp_path / "roaming" / "NostalgiaLauncher")
...
def test_cache_dir_linux_uses_xdg(fake_platform, monkeypatch, tmp_path):
    ...
    assert cache_dir() == str(tmp_path / "xcache" / "nostalgia-launcher")
...
def test_data_dir_linux_uses_xdg_data_home(
    ...
    assert data_dir() == str(tmp_path / "xdata" / "nostalgia-launcher")
...
def test_data_dir_windows_uses_localappdata(
    ...
    fake_platform("win32")
    monkeypatch.setenv("LOCALAPPDATA", str(tmp_path / "local"))
    assert data_dir() == str(tmp_path / "local" / "NostalgiaLauncher")
