...
@pytest.mark.parametrize(
    "marker,exe",
    [
        ("WoW.exe", "client/WoW.exe"),
        ("client.exe", "client/client.exe"),
        ("Launcher.exe", "client/Launcher.exe"),
    ],
)
def test_detect_torrent_root_uses_configured_marker(marker, exe):
    """The root marker is configurable (defaults to WoW.exe)."""
    ...
    class FakeFiles:
        ...
        def file_path(self, i):
            return [exe, "client/Data/a.bin", "client/Data/b.mpq"][i]

    root, mapping = td._detect_torrent_root(FakeFiles(), root_marker=marker)
    assert root == "client"
    assert mapping[exe] == os.path.basename(exe)
    assert mapping["client/Data/a.bin"] == "Data/a.bin"


def test_detect_torrent_root_custom_marker_missing():
    """A custom marker that isn't present → TorrentLayoutError."""
    ...
    with pytest.raises(td.TorrentLayoutError, match="no client.exe"):
        td._detect_torrent_root(FakeFiles(), root_marker="client.exe")


def test_configured_root_marker_from_launcher():
    """The configured torrent_root_marker is surfaced by the helper."""
    try:
        launcher.configure_from_dict(
            {
                "server": {
        ...
        assert td._configured_root_marker() == "client.exe"
    finally:
        launcher.reset()
...
def test_torrent_download_collects_stale_files(tmp_path, monkeypatch):
    ...
    assert any(
        isinstance(e, LogMessage) and "[torrent]" in e.text for e in events
    )
...
def test_torrent_download_falls_back_on_failure(tmp_path, monkeypatch):
    ...
    events = dispatcher.drain()
    assert any(
        isinstance(e, LogMessage) and "Falling back to HTTP" in e.text
        for e in events
    )
...
def test_run_invokes_torrent_then_skips_covered_files(tmp_path, monkeypatch):
    ...
    events = dispatcher.drain()
    assert any(isinstance(e, UpdateCompleted) for e in events)
...
def test_verify_worker_skips_rescan_when_torrent_unchanged(
    ...
    """When the snapshot at the URL is unchanged since the last verify (same
...
def test_verify_worker_skips_when_metadata_changes_but_identity_same(
    tmp_path, monkeypatch
):
    """A .torrent whose raw bytes changed (e.g. regenerated creation date)
    but whose info hash is identical carries the same piece hashes and file
...
def test_verify_worker_skips_when_url_rotates_same_snapshot(
    tmp_path, monkeypatch
):
    """Mirror failover serves the same snapshot from another URL — the exact
    ...
    snapshot = SimpleNamespace(
        url="https://mirror.example/client.torrent",
        content_hash="abc123",
        info_hash="ih1",
        torrent_bytes=b"",
        torrent_info=None,
...
def test_verify_worker_logs_full_rescan_reason_without_record(
    tmp_path, monkeypatch
):
