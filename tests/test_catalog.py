...
def test_validate_addon_slims_and_defaults():
    cleaned = catalog.validate_addon({
        "name": "pfUI", "git": "https://github.com/a/pfUI",
...
def test_validate_mod_github_release():
    cleaned = catalog.validate_mod({
        ...
        "source": {"kind": "github_release", "owner": "hannesmann",
                   "repo": "vanillafixes", "asset_pattern": "vanillafixes-*.zip",
                   "prefer_no": "-dxvk",
...
def test_validate_mod_rejects_unknown_kind():
    assert catalog.validate_mod(
        {"id": "X", "source": {"kind": "exec_arbitrary"}}) is None
...
def test_validate_mod_rejects_path_traversal_dest():
    assert catalog.validate_mod({
        "id": "X", "source": {"kind": "direct_file",
                              ...
                              "dest": "../../x.dll"}}) is None
    assert catalog.validate_mod({
        "id": "X", "source": {"kind": "github_release", "owner": "a",
                              "repo": "b", "asset_pattern": "*.zip",
                              "extract_map": {"x.dll": "../x.dll"}}}) is None


def test_validate_mod_rejects_bad_repo_url():
    assert catalog.validate_mod({
        "id": "X", "repo_url": "http://insecure.example",
        "source": {"kind": "direct_file", "url": "https://e/x.dll",
                   "dest": "x.dll"}})["repo_url"] is None
...
def test_merge_addons_custom_overrides_and_appends():
    remote = [
        {"name": "A", "git": "https://github.com/x/A", "branch": None,
         ...
         "blocked": False},
        {"name": "B", "git": "https://github.com/x/B", "branch": None,
         "ref": None, "description": None, "toc": {}, "recommended": False,
         "blocked": False},
    ]
    custom = [
...
def test_registry_url_override_and_clear(tmp_path):
    config_store.configure(str(tmp_path / "cfg.json"), str(tmp_path / "cache.json"))
    config_store.save_config({})
    assert catalog.get_registry_url("addons") == ""

    assert catalog.set_registry_url(
        "addons", "https://example.com/catalog.json") is None
    assert catalog.get_registry_url("addons") == "https://example.com/catalog.json"
...
def test_set_registry_url_rejects_insecure_and_credentials(tmp_path):
    ...
    assert catalog.set_registry_url(
        "mods", "https://user:pass@example.com/x") is not None
    assert catalog.get_registry_url("mods") == ""
...
def test_load_custom_skips_invalid_and_missing(tmp_path, monkeypatch):
    monkeypatch.setattr(catalog, "config_dir",
                        lambda: str(tmp_path))
    assert catalog.load_custom("addons", catalog.validate_addon) == []

    (tmp_path / "vanilla_wow_launcher_addons_custom.json").write_text(
        '[{"folder": "Good", "git": "https://github.com/a/b"}, '
        '{"folder": "../evil"}, "nope"]', encoding="utf-8")
