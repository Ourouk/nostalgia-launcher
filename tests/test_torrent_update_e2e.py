...
checked out under ``context/client`` and the real client snapshot at
...
def _local_snapshot(
    ...
    """Build a real ``TorrentSnapshot`` from the on-disk client ``.torrent``.
...
def test_e2e_reuses_cached_verdict_when_snapshot_unchanged(
    tmp_path, monkeypatch
):
    """When the snapshot at the URL is unchanged between two verifies, the
    second run reuses the cached verdict and skips the on-disk recheck — only
    one recheck runs. Uses an empty target so the recheck is instant."""
    _patch_source(monkeypatch)
    ...
    def counting(self, url, snapshot=None):
        calls["n"] += 1
        return real_verify(self, url, snapshot)
    ...
    # The recheck ran only on the first run; the second reused the cache.
    assert calls["n"] == 1
    assert any(
        "skipping re-scan and reusing cached verdict" in m
        for m in _messages(log_q)
    )
