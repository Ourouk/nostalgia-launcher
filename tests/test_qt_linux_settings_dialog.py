...
def test_umu_skip_builtin_dxvk_toggle_calls_setter(qapp, window, monkeypatch):
    hub = window._hub
    set_skip = Mock()
    monkeypatch.setattr(hub.settings, "set_umu_skip_builtin_dxvk", set_skip)
    monkeypatch.setattr(
        hub.settings,
        "linux_features",
        lambda: {"gamemode_available": True, "wayland_session": True},
    ...
    _, linux = _open_linux(window, monkeypatch)
    check = linux.findChild(QCheckBox, "linuxSettingsSkipBuiltinDxvk")
    assert check is not None
    check.setChecked(True)
    set_skip.assert_called_once_with(True)
