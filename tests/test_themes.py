...
def test_default_is_default_palette():
...
def test_non_dict_falls_back():
    for spec in ("bogus", 42, ["C_GOLD"], None):
...
def test_resolve_logo_accepts_https_url():
    assert (
        resolve_logo({"logo": " https://server.example/logo.png "})
        == "https://server.example/logo.png"
    )
...
def test_resolve_logo_rejects_non_https_url():
    for value in (
        "http://server.example/logo.png",
        "logo.png",
        "https://user:pass@server.example/logo.png",
    ):
