...
> **Historical note.** This review was written against the **pre-rename
> v1.4.1** codebase (package `vanilla_wow_launcher`, repo root
> `vanilla-wow-launcher`, application "Vanilla WoW Launcher"). The project was
> subsequently renamed to **Nostalgia Launcher** (`nostalgia_launcher` /
> `octo-updater` → `nostalgia-launcher`), the built-in server directory was
> removed in favor of user-imported configurations, private-server host
> references (`octowow.st`) were removed from allowlists, and the example
> configs were replaced with synthetic `examples/community.*.example.json`.
> File paths, symbols, and example filenames below reflect the older state and
> should be read as historical; verify against current source before acting.
...
`nostalgia_launcher.json` configuration file chosen by the user or the server.
...
├── NostalgiaLauncher.spec        # PyInstaller Windows onefile
├── NostalgiaLauncher-linux.spec # PyInstaller Linux onedir (AppImage source)
├── NostalgiaLauncher-macos.spec # PyInstaller macOS universal2 .app
├── packaging/
│   ├── pyinstaller_entry.py       # frozen entry shim → cli.main()
│   ├── fonts/STIXTwoMath-Regular.otf  # bundled glyph font
│   ├── icons/NostalgiaLauncher.png
│   ├── linux/{AppRun, NostalgiaLauncher.desktop, build-appimage.sh}
│   └── macos/{build-icons.sh, build-dmg.sh}
├── .github/workflows/{ci.yml, release.yml}
├── docs/{DEVELOPER.md, DISPLAY_TEST_MATRIX.md}
├── examples/{community.example.json, community.mods.example.json, community.addons.example.json}
...
│   ├── __main__.py                # `python -m nostalgia_launcher`
...
1. `pyproject` console script `nostalgia-launcher = nostalgia_launcher.cli:main`.
2. `python -m nostalgia_launcher` → `cli.main()` (`__main__.py` → `cli.main`).
...
3. First-run wizard (`launcher_config_dialog.py`):
   ...
   - User picks a remote server or a local `nostalgia_launcher.json`.
...
- Installs from Git host archives (`github/gitlab/gitea/codeberg`), plus any
  extra hosts named by the configuration's `addon_git_hosts`.
...
|------|----------|-------|-------|
| Launcher config | `<config_dir>/nostalgia_launcher.json` | the server config JSON | `core/launcher` |
| App config | `<config_dir>/nostalgia_launcher_config.json` | arbitrary JSON (see keys below) | `core/config_store` |
| Hash cache | `<cache_dir>/nostalgia_launcher_hash_cache.json` | `{path: [sha1_upper, mtime]}` + `__torrent_validation__` | `UpdateWorker`/`VerifyWorker` |
| Custom mods | `<config_dir>/nostalgia_launcher_mods_custom.json` | JSON list | `services/catalog` |
| Custom addons | `<config_dir>/nostalgia_launcher_addons_custom.json` | JSON list | `services/catalog` |
...
- `api.github.com/repos/Ourouk-nostalgia-launcher/releases/latest` (self-update).
...
- Linux: launch only via umu; config in `~/.nostalgia-launcher`.
...
- Optional `--launcher-config examples/community.example.json`.
...
- **`release.yml`** builds on `v*` tag push across Windows/Linux/macOS, each
  ...
  artifacts + `sha256` checksums. (Example configs are no longer shipped with
  releases; they live in the repo under `examples/`.)
...
- **Description:** `cmd = f"Add-MpPreference -ExclusionPath '{client_dir}'"`
  ...
  single quote (e.g. `C:\Users\O'Brien\Games\Nostalgia`) breaks out of the quote
...
**Consistency.**
- Relative imports inside the package; absolute `nostalgia_launcher.*` in
...
| K2 | `NostalgiaLauncher*.spec` (all 3) | `controllers.full_update` in `hiddenimports`; module removed by `FullUpdateController` revert (commit `582e93e`). **FIXED 2026-08-21**: stale hidden import removed from all three specs. | PyInstaller warning, never fails, but stale. | Harmless build noise. | **High** |
...
**Packaging change.** Edit the matching `NostalgiaLauncher*.spec`
...
- **Relative imports inside the package**; absolute `nostalgia_launcher.*`
...
2. `src/nostalgia_launcher/cli.py` — entry/startup/first-run.
3. `src/nostalgia_launcher/core/launcher.py` — config contract.
4. `src/nostalgia_launcher/core/security_http.py` — transfer security.
5. `src/nostalgia_launcher/state/events.py` + `state/models.py` — event bus
   + state.
6. `src/nostalgia_launcher/controllers/update.py` — update lifecycle +
   readiness.
...
3. `src/nostalgia_launcher/cli.py` (entry)
4. `src/nostalgia_launcher/core/launcher.py` (config)
5. `src/nostalgia_launcher/state/events.py` + `state/models.py`
6. `src/nostalgia_launcher/controllers/update.py` (lifecycle)
