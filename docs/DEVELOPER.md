# Nostalgia Launcher — Developer Guide
...
The launcher is a local tool. It does not include or recommend any server
...
- Addon Git hosts are restricted: a base allowlist (`github.com`,
  `gitlab.com`, `gitea.com`, `codeberg.org`) plus any extra hosts named by
  the configuration's `server.addon_git_hosts` (validated hostnames only, no
...
- Windows build: `uv run pyinstaller --noconfirm --clean NostalgiaLauncher.spec`
- Linux AppImage: `./packaging/linux/build-appimage.sh` →
  `dist/NostalgiaLauncher-$(uname -m).AppImage` (uses `linuxdeploy`, which
...
- macOS DMG (universal2, build on macOS): `./packaging/macos/build-dmg.sh` →
  `dist/NostalgiaLauncher-universal2.dmg` (requires a *universal*
