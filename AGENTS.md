...
Verification order for CI parity: `ruff check` → `ruff format --check` → `pyright` → `pytest -m "not e2e"` (see `.github/workflows/tests.yml`; `ci.yml`/`release.yml` call it). Pyright `include` is only `src/nostalgia_launcher`. CI pins CPython 3.12 (libtorrent ships wheels for 3.10–3.13 only; 3.14+ silently loses the torrent path). `tests/test_qml_lint.py` runs inside the normal suite but skips when no `qmllint` binary exists (Qt binary, not a PyPI package — never a CI gate).
...
- **Entry points:** the `nostalgia-launcher` console script and `python -m nostalgia_launcher` both reach `cli.main()`; the PyInstaller specs freeze `packaging/pyinstaller_entry.py` instead (relative imports break if `cli.py` is the frozen script). The QML shell (`NOSTALGIA_UI_BACKEND=qml`) is the only GUI backend since the widget tree was removed.
...
- **Layering:** `core/`, `services/`, `controllers/`, `state/` stay PySide6-free (tests import without Qt). Qt lives in `ui/` only (`qml/` view-models, `bridge.py`, `theme.py`, `metrics.py`, `app_lock_qt.py`, `relaunch.py`); keep Qt imports inside functions where a module must stay importable without Qt.
...
- **Hands-off:** `context/` (third-party refs, `server_list/*.json` reference server configs, real client for e2e) and `todo/` (local planning notes) are gitignored — never lint, execute, or commit them. `tmp/` (scratch repro/measurement scripts) is gitignored too — keep experiments there, delete when done.

## Testing quirks

- Run only covering test(s) while iterating; full `-m "not e2e"` once before commit.
- In this checkout `uv run pytest` silently collects zero tests (broken shim, exit 0 — false green). Run suites with `.venv/bin/python -m pytest` instead.
- **QML lint:** `qmllint` is a Qt binary, not a PyPI/uv package — neither the `pyside6` wheel nor any `pyside6-qmllint` package ships it (verified: no such PyPI package, no `Qt/bin/qmllint` in the wheel). `tests/test_qml_lint.py` assumes the binary is usable and skips with a clear message when it isn't. Candidate locations, first hit wins: `PATH`, `/usr/lib/qt6/bin/qmllint` (distro `qt6-declarative`):
  ```bash
  uv run python -m pytest tests/test_qml_lint.py   # skips if no qmllint
  ```
  Actionable classes only: `Quick.layout-positioning` (`height:` on layout-managed items → `Layout.preferredHeight`), real `missing-property`/`ReferenceError` risks. Ignore `unqualified` on engine context properties (`appTheme`, `*Model` — by design, root-context globals), `FileDialog.OpenDirectory` undefined offscreen (guarded at runtime), and `TypeError: Cannot read property … of null` at engine load (known Qt/offscreen artifact — bindings resolve post-load).
- QML conventions live in `docs/agents-ui.md`: no bare parent-scope properties (the `isRelease` ReferenceError class), `parent.*` never crosses layout boundaries, `onToggled` (not `onCheckedChanged`) for checkbox setters — except the ADDONS/MODS list delegates, which keep `onCheckedChanged` plus model-side echo guards. Static `objectName` (role-derived names are findChild-invisible, QTBUG). Settings/Log are modeless single-instance `ApplicationWindow`s (`showSettings()`/`showLog()`); one signal owner per action (duplicate QML+Python handlers on `logsRequested` double-toggled the dialog).
