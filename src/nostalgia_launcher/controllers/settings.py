...
class SettingsController:
    ...
    def set_umu_skip_builtin_dxvk(self, enabled: bool) -> dict:
        """Persist the skip-built-in-DXVK toggle: when on (with the DXVK
        renderer), Proton's stock DXVK is not staged so client-folder DLLs
        installed via MODS (e.g. dxvk-gplasync) are the ones used."""
        self._set_launch(
            lambda launch: launch.__setitem__(
                "umu_skip_builtin_dxvk", bool(enabled)
            )
        )
        return self.state.config
    ...
    def set_umu_game_id(self, game_id: str) -> dict:
        ...
        self._set_launch(
            lambda launch: launch.__setitem__(
                "umu_game_id",
                (game_id or "").strip() or "umu-nostalgia-launcher",
            )
        )
        return self.state.config
    ...
    def reload_addons_registry(self):
        ...
        threading.Thread(
            target=functools.partial(
                self._reload_catalog_worker,
                lambda: addons.fetch_addons_catalog(force=True),
                "Addon catalog",
                self._addons,
                self._addons.verify,
            ),
    ...
    def reload_mods_registry(self):
        ...
        threading.Thread(
            target=functools.partial(
                self._reload_catalog_worker,
                lambda: mods.fetch_mods_catalog(force=True),
                "Mod catalog",
                self._mods,
                self._mods.load_latest_versions,
            ),
    ...
    def _reload_catalog_worker(self, fetch, label, state, refresh):
        try:
            fetch()
        except Exception as e:
            self._dispatcher.post(
                LogMessage(
                    f"✗ {label} reload failed: {describe_net_error(e)}\n",
                    "err",
                )
            )
            return
        state.invalidate()
        refresh()
        self._dispatcher.post(LogMessage(f"✓ {label} reloaded.\n", "ok"))
