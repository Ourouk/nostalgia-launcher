"""Nostalgia Launcher Qt (PySide6) Linux settings dialog.
...
class LinuxSettingsDialog(QDialog):
    ...
    def _build_body(self) -> QVBoxLayout:
        ...
        dxvk_check = self._add_feature_check(
            layout=body,
            text="Client-folder DXVK (skip Proton's built-in)",
            object_name="linuxSettingsSkipBuiltinDxvk",
            checked=launch.umu_skip_builtin_dxvk,
            available=True,
            on_toggled=self._settings.set_umu_skip_builtin_dxvk,
            unavailable_hint="",
        )
        dxvk_check.setToolTip(
            "With the DXVK (D3D8) renderer: don't stage Proton's stock DXVK "
            "— use the d3d9.dll installed via MODS instead (e.g. "
            "dxvk-gplasync) and enable async shader compilation."
        )
