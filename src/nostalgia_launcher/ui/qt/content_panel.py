...
install checkbox, essential star badge, name/version, repo link, retry/update
action and error line, between a banner header and an Install-Essential/Apply
footer. This module holds that shared row widget and panel skeleton; each
...
class ContentListPanel(ScrollListPanel):
    ...
    def _build_footer(self, essential_objname: str):
        """The shared footer: '★ Install Essential' + Apply buttons."""
        self._add_hsep()
        footer = QWidget(self)
    ...
    def _essential_remaining(self) -> list:
        """Essential entries (registry flag) not yet present on disk."""
        remaining = []
        for entry in self._content_ctrl.registry:
            if not entry.get("essential", False):
                continue
    ...
    def _refresh_essential_visibility(self):
        ...
        least one essential entry not yet installed and no install is
