...
def build_env(
    ...
    Wayland backend (a no-op on X11). With `skip_builtin_dxvk` and the DXVK
    (D3D8) renderer, `PROTON_DXVK_D3D8` is omitted so Proton never stages its
    ...
    if renderer == RENDERER_DXVK_D3D8:
        if skip_builtin_dxvk:
            # Both spellings: gplasync builds read DXVK_GPLASYNC, older
            # dxvk-async builds read DXVK_ASYNC; each ignores the other.
            env["DXVK_GPLASYNC"] = "1"
            env["DXVK_ASYNC"] = "1"
        else:
